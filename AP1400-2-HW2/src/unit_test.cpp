...
TEST(HW1Test, TEST9) {
  ...
  EXPECT_FALSE(
      crypto::verifySignature(p->get_publickey(), "notmydata", signature));
...
TEST(HW1Test, TEST15) {
    ...
    bryan->get_wallet()==4.5); EXPECT_TRUE(clint->get_wallet()==3.5
    ||clint->get_wallet()==3.5 ||clint->get_wallet()==9.75);
